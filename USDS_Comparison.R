...
USDS_Differences <- data.frame(Variable=character(), Difference=character(), p.value=numeric())

for (var in testVars){
  cat("T-test for", var, "between US and DS\n")
  ttest_result <- t.test(data[[var]] ~ data$SiteGroup)

  if (ttest_result$p.value < 0.05) {
    cat("Reject the null hypothesis, there is a significant difference between US and DS\n")
    USDS_Differences <- rbind(USDS_Differences, data.frame(Variable=var, Difference="Significant", p.value=ttest_result$p.value))
  } else {
    cat("Fail to reject the null hypothesis, there is no significant difference between US and DS\n")
    USDS_Differences <- rbind(USDS_Differences, data.frame(Variable=var, Difference="Not Significant", p.value=ttest_result$p.value))
  }
}
